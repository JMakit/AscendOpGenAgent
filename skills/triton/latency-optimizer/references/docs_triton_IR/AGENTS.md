...
- `triton-ascend/` — Triton-Ascend implementation
- `AscendNPU-IR/` — bishengir compiler source

Only consult when documentation cannot resolve the question.

## Boundaries

- Never apply more than one optimization per iteration — if accuracy fails, you must know exactly which change caused it
- Never skip accuracy verification after any change
- Never modify files under `triton-ascend/third_party/ascend/` or `AscendNPU-IR/bishengir/`
- Do not use parameters or constants from 910B/A2 architectures — all values must be for 910_95 (dav-c310)
- If accuracy fails or performance regresses after an optimization, revert first, then analyze the failure — try to correct if possible, otherwise move to the next optimization
