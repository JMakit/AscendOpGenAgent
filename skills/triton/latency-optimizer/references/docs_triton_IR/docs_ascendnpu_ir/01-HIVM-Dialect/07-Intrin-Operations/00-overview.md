...
HIVM 内建指令操作（Intrinsic Operations）是 HIVM 方言中最底层的操作，直接对应 NPU 硬件指令。这些操作从 [HIVMIntrinOps.td](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/include/bishengir/Dialect/HIVM/IR/HIVMIntrinOps.td) 定义，继承自 `LLVM_IntrOpBase`，在 Lowering 的最终阶段由高层 HIVM 操作转换而来。
...
- 源码参考：[HIVMIntrinOps.td](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/include/bishengir/Dialect/HIVM/IR/HIVMIntrinOps.td)
