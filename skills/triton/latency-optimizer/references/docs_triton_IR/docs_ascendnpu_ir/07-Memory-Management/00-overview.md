...
源码参考：[HIVMAttrs.td](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/include/bishengir/Dialect/HIVM/IR/HIVMAttrs.td#L171-L188)

## 内存规划中的 Inplace 复用

PlanMemory Pass 的核心优化之一是 Inplace 复用——当两个缓冲区的生命周期不重叠时，它们可以共享同一块物理内存。从 [plan-memory.mlir](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/test/Dialect/HIVM/plan-memory.mlir) 测试用例可以看到：
...
- 属性定义：[HIVMAttrs.td](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/include/bishengir/Dialect/HIVM/IR/HIVMAttrs.td)
- 内存规划测试：[plan-memory.mlir](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/test/Dialect/HIVM/plan-memory.mlir)
- 硬件规格定义：[NPUTargetSpec.td](file:///d:/项目/trae/triton_a5/AscendNPU-IR/bishengir/include/bishengir/Dialect/HACC/Targets/NPUTargetSpec.td)
