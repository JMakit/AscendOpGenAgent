...
Triton IR → Linalg IR 转换是 Triton-Ascend SIMD 编译路径的核心阶段，将 Triton Dialect 的操作转换为 MLIR 标准的 Linalg Dialect 操作。该转换由 [ttir_to_linalg()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L95) 函数驱动，在 Ascend 特有 Pass 之后执行 `TritonToLinalg` Pass 完成。
...
[ttir_to_linalg()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L95) 是 Ascend Pass 序列的入口函数，负责将 TTIR 转换为 Linalg IR：
...
[_parse_linalg_metadata()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L288) 从 Linalg IR 字符串中提取编译所需的元数据：
...
[_parse_ttir_metadata()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L342) 用于 SIMT 路径（TTIR 直接编译），从 TTIR 中解析元数据：
