...
TTIR 优化阶段在 AST → TTIR 生成之后执行，对 Triton IR 进行一系列与硬件无关的通用优化。这些优化 Pass 由 [make_ttir()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L72) 函数驱动，使用 MLIR 的 Pass Manager 框架按顺序执行。
...
[make_ttir()](file:///d:/项目/trae/triton_a5/triton-ascend/third_party/ascend/backend/compiler.py#L72) 函数定义了 TTIR 优化 Pass 的执行序列：
